...
# # Check for selected stock in session state
# if 'selected_stock' in st.session_state:
#     symbol = st.session_state.selected_stock
# else:
#     symbol = "DefaultSymbol"  # Replace with a default value or handle the absence of a selected stock
...
# Check if there's a symbol passed from the watch list
if 'selected_stock' in st.session_state:
    symbol_input = st.session_state.selected_stock
    st.session_state['btn_clicked'] = True
    st.session_state.pop('selected_stock')  # Remove it after using
else:
    symbol_input = st.text_input("Enter a stock ticker").upper()
...
if st.button('Go',on_click=callback) or st.session_state['btn_clicked']:
    ...
    def save_to_watch_list():
        if symbol_input not in st.session_state.watch_list:
            st.session_state.watch_list.append(symbol_input)
            st.success(f"Added {symbol_input} to watch list!")
